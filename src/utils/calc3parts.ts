...
export async function calculateThreeParts(input: CalcInput): Promise<CalcResult> {
  ...
  try {
    ...
    const { data: transport } = await selectWithFallback(TABLE_KEYS.transport, (q) => {
      let base = q
        .select('pick_up_location,delivery_location,direction,vehicle_type,charge_description,20gp,40gp_40hc,currency,dg_surcharge,transport_vendor,drop_trailer,heavy_weight_surcharge,tail_gate,side_loader_access_fees,container_unpack_rate_loose,container_unpack_rate_palletized')
    ...
    (transport ?? []).forEach((r: any) => {
      ...
      const fumigationSurcharge = input.fumigationSurcharge ? (parseFloat(r.fumigation_bmsb) || 0) : 0;
      
      // 20GP transport
      if (qty20 > 0) {
        const baseRate = parseFloat(r['20gp']) || 0;
        const rate = baseRate + dgSurcharge + dropTrailerCharge + heavyWeightSurcharge + tailgateCharge + sideLoaderAccessFees + unpackLooseCharge + unpackPalletizedCharge + fumigationSurcharge;
        if (rate > 0) {
          const total = rate * qty20;
          ...
          if (fumigationSurcharge > 0) additionalCharges.push(`Fumigation: ${fumigationSurcharge.toFixed(2)}`);
      ...
      if (lclCbm > 0) {
        ...
        if (cubicRate > 0) {
          const rate = cubicRate + dgSurcharge + dropTrailerCharge + heavyWeightSurcharge + tailgateCharge + sideLoaderAccessFees + unpackLooseCharge + unpackPalletizedCharge + fumigationSurcharge;
